-- Task module integrity hardening. Idempotent and safe to run at startup.

-- Legacy assignees/collaborators implicitly had project access. Persist that
-- access explicitly so membership, authorization and the UI share one model.
...
-- An orphan cannot be retained once the self-referencing FK is installed.
UPDATE tasks child
...
FOR EACH ROW
EXECUTE FUNCTION prevent_task_parent_cycle();
