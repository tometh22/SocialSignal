-- Feedback Mind V2: invariantes de miembros, fechas civiles y tareas internas.
...
SELECT 'Epical'
WHERE NOT EXISTS (
  SELECT 1 FROM clients WHERE name = 'Epical'
);
...
WHERE parent_task_id IS NULL
  AND (
    ...
    OR NOT EXISTS (
      SELECT 1
      FROM active_projects existing_project
...
-- Preserve pre-existing single estimates by assigning them to the task's
-- first relevant week. New writes use task_weekly_estimates exclusively.
...
WHERE task.estimated_hours IS NOT NULL
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM task_weekly_estimates estimate
...
-- Reconciliar el total USD del ledger. Cuando existen ARS y cotización, esa
-- identidad prevalece si el total guardado difiere más de 20%; evita rotular
-- ARS o valores multiplicados como dólares.
