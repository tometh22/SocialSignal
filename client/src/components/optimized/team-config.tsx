          ...
          <div className="mb-4 border border-gray-200 rounded-md overflow-hidden">
            ...
            <div className="p-3">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                ...
                {/* Selector de personal simplificado */}
                <div>
                  <Label htmlFor="personal-selector" className="text-xs mb-1 inline-block">Personal</Label>
                  
                  {/* Implementando una solución diferente */}
                  <div className="relative">
                    <select 
                      id="personal-selector"
                      className="w-full h-8 text-xs border border-gray-300 rounded-md px-2 py-1 appearance-none bg-white"
                      value={newMember.personnelId || ''}
                      onChange={(e) => {
                        const personnelId = parseInt(e.target.value);
                        if (isNaN(personnelId)) return;
                    ...
                    >
                      ...
                      {availablePersonnel && availablePersonnel.map(person => {
                    ...
                    <div className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                      <svg className="h-3 w-3 text-gray-500" fill="none" viewBox="0 0 12 12" stroke="currentColor">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        const role = availableRoles?.find(r => r.id === roleId);
                        if (!role) {
                          console.log("No se encontró el rol seleccionado");
                          return;
                        }
                        
                        // Al seleccionar un rol, buscar personal disponible para este rol
                        ...
                        // Si hay personal disponible para este rol, seleccionar el primero
                        const firstPersonnel = personnelForRole.length > 0 ? personnelForRole[0] : null;
                        
                        setNewMember(prev => ({
                          ...prev,
                          roleId,
                          personnelId: firstPersonnel?.id || null,
                          rate: role?.defaultRate || 0
                        }));
                      }}
                    >
                ...
                {/* Original multiselection - dejamos este código comentado por ahora */}
                {/* 
                <div>
