...
const OptimizedFinancialReview: React.FC = () => {
  ...
  // Funciones para editar miembros del equipo y personalización
  const {
    updateTeamMember, 
    removeTeamMember, 
    addTeamMember, 
    ...
    <div className="p-6 space-y-6 w-full max-w-full">
      ...
      <Card className="mt-4">
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Agrega detalles específicos para este proyecto
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="border rounded-md bg-white focus-within:ring-1 focus-within:ring-primary">
            <textarea 
              placeholder="Especifica detalles particulares, requerimientos especiales o consideraciones adicionales para este proyecto..."
              className="w-full px-3 py-2 min-h-[120px] rounded-md text-sm focus:outline-none resize-y"
              value={quotationData.customization || ''}
              onChange={(e) => {
                updateCustomization(e.target.value);
              }}
            />
          </div>
        </CardContent>
      ...
      {/* Alerta de personalización no especificada (si corresponde) */}
      {quotationData.template && !quotationData.customization && (
        <Alert className="bg-amber-50 text-amber-800 border-amber-200 mt-3">
