...
export default function EmptyReviewsState({ onCreate }: Props) {
  return (
    <div className="max-w-2xl mx-auto text-center py-16 px-6">
      ...
      <h2 className="text-2xl font-semibold text-slate-900 mb-3">Creá tu primer Review</h2>
      <p className="text-slate-600 leading-relaxed mb-4">
        Un Review es una <strong>sala colaborativa</strong> para hacer seguimiento semanal con un grupo específico:
        tu manager, tu equipo, un cliente.
      </p>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 text-left text-sm text-slate-600 mb-6">
        ...
        <ul className="space-y-1.5">
          <li>• <em>Review con COO</em> — proyectos top y decisiones pendientes</li>
          <li>• <em>Sync de Diseño</em> — prioridades del equipo</li>
          <li>• <em>Review cliente Warner</em> — estado de campañas con el cliente</li>
        </ul>
      </div>

      <Button size="lg" onClick={onCreate}>
        <Plus className="h-4 w-4 mr-2" />
        Crear mi primer Review
      </Button>
    </div>
  );
}
