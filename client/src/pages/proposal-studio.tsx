...
export default function ProposalStudio() {
  ...
  return (
    <main className="min-h-screen bg-slate-100">
      ...
      <div className="mx-auto grid max-w-[1680px] gap-5 p-5 xl:grid-cols-[320px_minmax(0,1fr)_320px]">
        <aside className="space-y-4">
          <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Bot className="h-4 w-4 text-indigo-600" /> Editor agéntico</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-xs text-slate-500">Puede mejorar narrativa, jerarquía y tema. No recibe costos internos ni puede cambiar precio, horas, SLA o términos.</p><Textarea rows={7} value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Ej.: resumí el contexto, hacé más ejecutivo el cierre y ordená la historia para un CMO." /><Button className="w-full" variant="secondary" disabled={instruction.trim().length < 5 || propose.isPending || document.isStale} onClick={() => propose.mutate()}><Sparkles className="mr-2 h-4 w-4" /> Proponer cambios</Button>{agentProposal && <Alert><AlertTitle>{agentProposal.patch.summary}</AlertTitle><AlertDescription className="mt-2 space-y-3"><p>{agentProposal.patch.operations.length} operaciones editoriales tipadas.</p><div className="flex gap-2"><Button size="sm" onClick={() => decide.mutate("accept")}><Check className="mr-1 h-3.5 w-3.5" /> Aplicar</Button><Button size="sm" variant="outline" onClick={() => decide.mutate("reject")}><X className="mr-1 h-3.5 w-3.5" /> Descartar</Button></div></AlertDescription></Alert>}</CardContent></Card>
          <Card><CardHeader><CardTitle className="text-sm">Principio de seguridad</CardTitle></CardHeader><CardContent className="text-xs leading-5 text-slate-500">Todo cambio comercial vuelve al cotizador, se confirma explícitamente y recalcula. El agente de este Studio sólo opera el documento sanitizado.</CardContent></Card>
        ...
        <section className="space-y-4">
          {content.blocks.map((block, index) => (
            <Card key={block.id} className={`${block.visible ? "" : "opacity-60"} overflow-hidden`}>
              <div className="flex items-center justify-between border-b bg-slate-50 px-4 py-2"><div className="flex items-center gap-2"><Badge variant="outline">{block.type}</Badge>{block.internalOnly && <Badge variant="destructive">Interno</Badge>}{PROTECTED_BLOCK_TYPES.has(block.type) && <Badge variant="secondary">Sincronizado con cotización</Badge>}</div><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => moveBlock(index, -1)} disabled={index === 0}><ArrowUp className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => moveBlock(index, 1)} disabled={index === content.blocks.length - 1}><ArrowDown className="h-4 w-4" /></Button><Button variant="ghost" size="icon" disabled={PROTECTED_BLOCK_TYPES.has(block.type)} onClick={() => updateBlock(block.id, { visible: !block.visible })}>{block.visible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}</Button></div></div>
              <CardContent className="space-y-3 p-6"><Input disabled={PROTECTED_BLOCK_TYPES.has(block.type)} className="border-0 px-0 text-xl font-semibold shadow-none focus-visible:ring-0 disabled:opacity-100" value={block.title} onChange={(event) => updateBlock(block.id, { title: event.target.value })} /><Textarea disabled={PROTECTED_BLOCK_TYPES.has(block.type)} className="min-h-24 resize-y border-slate-200 leading-6 disabled:opacity-100" value={block.body ?? ""} onChange={(event) => updateBlock(block.id, { body: event.target.value })} placeholder="Narrativa del bloque" /><div><Label className="text-xs text-slate-500">Bullets · uno por línea</Label><Textarea disabled={PROTECTED_BLOCK_TYPES.has(block.type)} className="mt-1 min-h-24 disabled:opacity-100" value={block.bullets.join("\n")} onChange={(event) => updateBlock(block.id, { bullets: event.target.value.split("\n").map((line) => line.trim()).filter(Boolean) })} /></div></CardContent>
            </Card>
          ))}
        </section>

        <aside className="space-y-4">
          <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Palette className="h-4 w-4 text-indigo-600" /> Identidad visual</CardTitle></CardHeader><CardContent className="space-y-3">{([['primaryColor','Color principal'],['accentColor','Acento'],['backgroundColor','Fondo']] as const).map(([key,label]) => <div key={key}><Label className="text-xs">{label}</Label><div className="mt-1 flex gap-2"><Input type="color" className="h-9 w-12 p-1" value={content.theme[key]} onChange={(event) => setDraft({ ...content, theme: { ...content.theme, [key]: event.target.value } })} /><Input value={content.theme[key]} onChange={(event) => setDraft({ ...content, theme: { ...content.theme, [key]: event.target.value } })} /></div></div>)}<div><Label className="text-xs">Tipografía</Label><Input className="mt-1" value={content.theme.fontFamily} onChange={(event) => setDraft({ ...content, theme: { ...content.theme, fontFamily: event.target.value } })} /></div><div className="grid grid-cols-2 gap-2"><Label className="cursor-pointer rounded-md border px-3 py-2 text-center text-xs hover:bg-slate-50">Logo cliente<Input className="hidden" type="file" accept="image/png,image/jpeg" disabled={uploadingAsset} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAsset(file, "client_logo"); }} /></Label><Label className="cursor-pointer rounded-md border px-3 py-2 text-center text-xs hover:bg-slate-50">Imagen de marca<Input className="hidden" type="file" accept="image/png,image/jpeg" disabled={uploadingAsset} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAsset(file, "brand_image"); }} /></Label></div><p className="text-xs text-slate-500">{content.assets.length} recursos vinculados al documento.</p></CardContent></Card>
          <Card><CardHeader><CardTitle className="text-base">Control de calidad</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex gap-2"><Badge variant={blockerCount ? "destructive" : "secondary"}>{blockerCount} bloqueos</Badge><Badge variant="outline">{issues.length - blockerCount} advertencias</Badge></div>{issues.length === 0 ? <p className="text-xs text-slate-500">Ejecutá QA luego de guardar.</p> : <ul className="space-y-2">{issues.map((issue, index) => <li key={`${issue.code}-${index}`} className="rounded-lg border p-2 text-xs"><strong>{issue.code}</strong><p className="mt-1 text-slate-500">{issue.message}</p></li>)}</ul>}</CardContent></Card>
          <Card><CardHeader><CardTitle className="text-base">Exportar</CardTitle></CardHeader><CardContent className="grid gap-2"><Button variant="outline" disabled={document.isStale || document.status === "draft"} onClick={() => window.open(`/api/quotations/${quotationId}/proposal-documents/${document.id}/export.pdf`, "_blank")}><Download className="mr-2 h-4 w-4" /> PDF modular</Button><Button variant="outline" disabled={document.isStale || document.status === "draft"} onClick={() => window.open(`/api/quotations/${quotationId}/proposal-documents/${document.id}/export.pptx`, "_blank")}><Download className="mr-2 h-4 w-4" /> PPTX editable</Button></CardContent></Card>
        </aside>
      </div>
    </main>
  );
