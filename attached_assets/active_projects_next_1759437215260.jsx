  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-2">
        ...
        />
        <button onClick={onRefresh} className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm font-medium text-indigo-700 hover:bg-indigo-100">
...
function ActiveProjectsInner(){
  const [period, setPeriod] = useState<string>(lastMonthKey());
  ...
  return (
    <div className="mx-auto max-w-6xl p-5 sm:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500">Unified dashboard powered by Excel/DB SoT</p>
      </div>
      ...
      <div className="mt-6 text-xs text-slate-400">
        <div>Period: <span className="font-mono">{data?.period ?? period}</span> • Updated: {data?.updatedAt ? new Date(data.updatedAt).toLocaleString() : "—"}</div>
      </div>
    </div>
  );
}
