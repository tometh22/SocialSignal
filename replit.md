...
- **Quotation Dual-Currency Rate System**: Roles have `defaultRate` (ARS) and `defaultRateUsd` (USD). Personnel have historical monthly ARS rates and `hourlyRate` (USD). Quotation `quotationCurrency` field ('ARS'|'USD') controls which rates are used. `getPersonnelRate(id, currency)` returns the correct rate. `updateQuotationCurrency()` recalculates all team member rates when switching. Admin panel shows both ARS and USD rate columns for roles and personnel. All display pages (team config, financial review, quote list, quote detail) dynamically show the quotation's selected currency.
