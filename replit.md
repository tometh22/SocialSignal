...
- **Deterministic + Fuzzy Project Resolver V2 (Oct 2025)**: Comprehensive project matching system for SoT ETL with 3-stage cascade resolution. Architecture: (1) **dim_client_alias** - normalized client name mappings with fuzzy auto-creation (threshold ≥0.92 using fuse.js), storing canonical clientId references; (2) **dim_project_alias** - normalized project name mappings scoped by clientId with exact match → token subset → fuzzy cascade, auto-creating successful matches; (3) **rc_unmatched_staging** - audit table logging all unmatchable RC rows with motivo (unknown_client, unknown_project, below_threshold), fuzzy scores, and candidate suggestions. Resolution flow: `resolveRCRow()` normalizes inputs via `normKey()` (lowercase, no accents, trim) → resolves clientId (exact cache → fuzzy ≥0.92 → auto-alias) → resolves projectId using clientId to filter candidates (exact cache → token subset unique match → fuzzy ≥0.92 → auto-alias) → logs staging if failed. In-memory cache with invalidation on new alias creation. Management endpoints: GET `/api/etl/sot/diagnostics/rc-unmatched?period=YYYY-MM` (view unmatched rows with summary), POST `/api/etl/sot/reprocess-rc` (re-run ETL for specific periods with fresh Google Sheets fetch and optional aggregate recomputation), POST `/api/etl/sot/migrate-aliases` (one-time migration from legacy `project_aliases` table). System learns from Excel data automatically, reducing manual alias maintenance while maintaining full audit trail of unresolved cases.
...
- **fuse.js**: For fuzzy string matching in project resolver (≥0.92 threshold).