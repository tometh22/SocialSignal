...
- July 30, 2025: **PROJECT TEAM CONFIGURATION SYSTEM IMPLEMENTED** - Added comprehensive team configuration functionality for role-based quotations: created ProjectTeamConfiguration component that intelligently detects quotations made with roles only, provides professional modal interface for assigning actual personnel to roles with rate adjustments, integrated into project creation workflow in manage-quotes.tsx with automatic detection and seamless fallback, added backend API endpoint `/api/quotation-team/:quotationId/assign-personnel` for updating team assignments, ensures smooth transition from role-based quotations to personnel-assigned projects without breaking existing functionality
- July 30, 2025: **QUOTATION TEAM MEMBER PERSISTENCE FIX** - Fixed critical database constraint issue preventing team members from being saved: altered quotation_team_members table to allow NULL values in personnel_id column (was incorrectly NOT NULL), verified fix with comprehensive test that successfully saved 2 team members with role-only assignments, ensures quotations can now be saved and edited with proper team member persistence
- July 30, 2025: **MARKUP DISPLAY FORMAT FIX** - Fixed markup display in quotation cards showing "2x6" instead of "2.6x": changed from toLocaleString('es-ES') which uses comma decimal separator to toFixed(1) for consistent decimal point format, ensures markup multipliers display correctly as "2.6x" across all quotation cards
- July 30, 2025: **CRITICAL FIX: ROLE-ONLY QUOTATIONS DATA INTEGRITY** - Fixed critical bug where role-only quotations were being corrupted by auto-assignment of personnel: removed automatic personnel assignment in backend POST /api/quotation-team route that was force-creating generic personnel, updated getQuotationTeamMembers to respect null personnelId values without forcing default values, enhanced UI to show "Sin asignar" (unassigned) for role-only team members, ensures quotations maintain their original intent until project creation phase where proper personnel assignment should occur, prevents data corruption and maintains business logic integrity for role-based quotations
- July 30, 2025: **QUOTATION CARDS ENHANCED WITH FINANCIAL METRICS** - Complete redesign of quotation management cards: added visual status color indicators on left edge, increased logo size to 14x14px with better presentation, added comprehensive information display including creation date, project type (Always-On/One-Shot), team member count, enhanced pricing section now shows client price, base cost, and markup percentage with color coding (green ≥2.5x, blue ≥2.0x, amber ≥1.5x, red <1.5x), improved spacing with p-5 padding and professional hover effects, action buttons now include descriptive text for better UX, enables quick financial assessment without opening details
- July 29, 2025: **QUOTATION DRAFT SAVE FIX** - Fixed critical error preventing draft saving in final quotation step: corrected proposalLink validation schema from strict `z.string` to `z.string().nullable().optional()` allowing null values, resolved Zod validation error that was blocking PUT requests to save drafts, ensures users can save progress at any stage of quotation creation process without data loss
- July 29, 2025: **PROJECT TYPE BADGES AND DATE DISPLAY FIX** - Added comprehensive project type badges to active projects view showing "Always-On" (purple), "Monitoring" (orange), or "One-Shot" (blue) based on quotation projectType field, fixed critical date display issue where dates were showing one day earlier due to timezone handling by adding explicit UTC timezone handling for date formatting, enhanced visual distinction between different project types for better project portfolio management
- July 29, 2025: **EXECUTIVE DASHBOARD COMPLETELY REDESIGNED AS TRUE HOME PAGE** - Replaced redundant executive dashboard with focused home page design: new daily snapshot showing today's metrics (hours worked, active people, deliverables completed), quick actions panel for most frequent tasks (new quotation, register time, manage deliverables), intelligent alerts system showing only critical business issues (inactive projects, old pending quotes, budget risks), weekly activity trend chart, recent activity feed showing last 48 hours of system movements, eliminated all duplication with Analytics & Reportes section, dashboard now serves as true operational command center for daily management
