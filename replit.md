...
- **Quotation Currency Standardization**: All quotation system values (roles, personnel rates, costs, totals) are standardized to ARS (pesos argentinos). Role `defaultRate`, personnel `hourlyRate`, and all cost calculations use ARS. No hardcoded USD-to-ARS conversions. Admin panel, team config, financial review, quote list, and quote detail all display ARS consistently.
