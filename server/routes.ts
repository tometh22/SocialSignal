...
// Sistema de limitación de tasa ultra-agresivo para prevenir bucles
// Mapa para seguir el número de peticiones por ruta
const requestCounts = new Map<string, {count: number, timestamp: number, blocked: boolean}>();
const RATE_LIMIT_WINDOW = 5000; // 5 segundos
const RATE_LIMIT_MAX = 3; // Máximo 3 peticiones por ruta en cada 5 segundos
const BLOCK_DURATION = 10000; // 10 segundos de bloqueo si se excede el límite

// Middleware para limitar peticiones API y prevenir bucles infinitos
const rateLimiter = (req: Request, res: Response, next: NextFunction) => {
  // Si es el ping endpoint, siempre permitir
  if (req.path === '/api/ping') {
    return next();
  }
  
  // Normalizar la URL para agrupar peticiones similares
  const path = req.path.replace(/\/\d+$/, '/*');
  const key = path;
  const now = Date.now();
  
  // Verificar si la ruta está bloqueada por limitación
  let requestData = requestCounts.get(key);
  
  if (requestData && requestData.blocked) {
    if (now - requestData.timestamp < BLOCK_DURATION) {
      console.warn(`[RATE LIMIT] Ruta ${path} bloqueada temporalmente`);
      return res.status(429).json({
        error: "Demasiadas peticiones. Intente nuevamente en unos segundos.",
        rateLimited: true
      });
    } else {
      // Si ha pasado el tiempo de bloqueo, reiniciar
      requestData.blocked = false;
      requestData.count = 1;
      requestData.timestamp = now;
    }
  } else if (!requestData) {
    // Primera petición para esta ruta
    requestData = { count: 1, timestamp: now, blocked: false };
    ...
    if (requestData.count > RATE_LIMIT_MAX) {
      console.warn(`[RATE LIMIT] Ruta ${path} ha excedido el límite (${requestData.count} peticiones en ${RATE_LIMIT_WINDOW}ms). Bloqueando temporalmente.`);
  ...
  // Limpiar periódicamente el mapa para evitar crecimiento ilimitado
  if (Math.random() < 0.01) { // ~1% de probabilidad por petición
    const cleanupTime = now - RATE_LIMIT_WINDOW * 2;
    for (const [mapKey, data] of requestCounts.entries()) {
      if (!data.blocked && data.timestamp < cleanupTime) {
...
export async function registerRoutes(app: Express): Promise<Server> {
  // Aplicar el limitador de tasa a todas las rutas API
