...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Alias route for project details (for frontend compatibility)
  app.get("/api/projects/:id/details", requireAuth, async (req, res) => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) return res.status(400).json({ message: "Invalid project ID" });
    ...
    try {
      ...
      // Calculate estimated hours from team members
      let estimatedHours = 0;
      if (project.quotation) {
        try {
          const teamMembers = await storage.getQuotationTeamMembers(project.quotation.id);
          ...
          console.error("Error calculating estimated hours:", error);
        }
      }
      ...
      // If it's a macro project, get its subprojects
      if (project.isAlwaysOnMacro) {
        try {
          const subprojects = await storage.getSubprojectsByParentId(id);
          return res.json({
            ...
            clientName: project.quotation?.client?.name || "Cliente desconocido",
            startDate: project.createdAt,
            endDate: project.quotation?.deliveryDate || null,
            totalBudget: project.quotation?.totalAmount || 0,
            budget: project.quotation?.totalAmount || 0, // Using dynamic field
            spentBudget: timeEntries.reduce((total, entry) => total + (entry.cost || 0), 0),
