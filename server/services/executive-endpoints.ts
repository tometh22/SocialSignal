...
export async function getFinancieroData(periodKeys: string[]): Promise<FinancieroData> {
  const lastPeriodKey = periodKeys[periodKeys.length - 1];
  
  const { rows: [excelData] } = await pool.query(`
    SELECT facturacion_total, ebit_operativo, beneficio_neto
  ...
  return {
    ...
    formula: {
      ...
      beneficioNeto: 'EBIT Contable - Ajustes PnL'
    },
    source: {
